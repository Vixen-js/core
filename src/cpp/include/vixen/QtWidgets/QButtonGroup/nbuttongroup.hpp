  ...
  public:
    ...
    virtual void connectSignalsToEventEmitter()
    {
        ...
        connect(this, &QButtonGroup::idClicked, [=](int id) {
            ...
            this->emitOnNode.Call({Napi::String::New(env, "onClick"), Napi::Number::New(env, id)});
        });
        connect(this, &QButtonGroup::idPressed, [=](int id) {
            Napi::Env env = this->emitOnNode.Env();
            Napi::HandleScope scope(env);
            this->emitOnNode.Call({Napi::String::New(env, "onMousedown"), Napi::Number::New(env, id)});
        });
        connect(this, &QButtonGroup::idReleased, [=](int id) {
            Napi::Env env = this->emitOnNode.Env();
            Napi::HandleScope scope(env);
            this->emitOnNode.Call({Napi::String::New(env, "onMouseup"), Napi::Number::New(env, id)});
        });
        connect(this, &QButtonGroup::idToggled, [=](int id, bool checked) {
            Napi::Env env = this->emitOnNode.Env();
            Napi::HandleScope scope(env);
            this->emitOnNode.Call(
                {Napi::String::New(env, "onToggle"), Napi::Number::New(env, id), Napi::Boolean::New(env, checked)});
        });
    }
};
