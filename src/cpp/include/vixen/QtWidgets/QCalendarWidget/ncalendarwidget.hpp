  ...
  public:
    ...
    virtual void connectSignalsToEventEmitter()
    {
        ...
        QObject::connect(this, &QCalendarWidget::activated, [=](const QDate &date) {
            ...
            this->emitOnNode.Call({Napi::String::New(env, "onActivate"), instance});
        });
        QObject::connect(this, &QCalendarWidget::clicked, [=](const QDate &date) {
            ...
            auto instance = QDateWrap::constructor.New(
                {Napi::External<QDate>::New(env, new QDate(date.year(), date.month(), date.day()))});
            this->emitOnNode.Call({Napi::String::New(env, "onClick"), instance});
        });
        QObject::connect(this, &QCalendarWidget::currentPageChanged, [=](int year, int month) {
            Napi::Env env = this->emitOnNode.Env();
            Napi::HandleScope scope(env);
            this->emitOnNode.Call({Napi::String::New(env, "onCurrentPageChange"), Napi::Value::From(env, year),
