...
Napi::Object QCalendarWidgetWrap::init(Napi::Env env, Napi::Object exports)
{
    ...
    Napi::Function func = DefineClass(env, CLASSNAME,
                                      {InstanceMethod("monthShown", &QCalendarWidgetWrap::monthShown),
                                       ...
                                       InstanceMethod("showPreviousMonth", &QCalendarWidgetWrap::showPreviousMonth),
                                       InstanceMethod("showPreviousYear", &QCalendarWidgetWrap::showPreviousYear),
                                       InstanceMethod("showSelectedDate", &QCalendarWidgetWrap::showSelectedDate),
...
QCalendarWidgetWrap::QCalendarWidgetWrap(const Napi::CallbackInfo &info) : Napi::ObjectWrap<QCalendarWidgetWrap>(info)
{
    ...
    else if (argCount == 1)
    {
        ...
        else
        {
            ...
            this->instance = new NCalendarWidget(parentWidgetWrap->getInternalInstance());
        }
    }
    else
    {
        Napi::TypeError::New(env, "Vixen: QCalendarWidgetWrap: Wrong number of "
...
Napi::Value QCalendarWidgetWrap::setCurrentPage(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
...
Napi::Value QCalendarWidgetWrap::showPreviousMonth(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
...
Napi::Value QCalendarWidgetWrap::showPreviousYear(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
...
Napi::Value QCalendarWidgetWrap::showSelectedDate(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
