    ...
    Napi::Function func =
        DefineClass(env, CLASSNAME,
                    {InstanceMethod("selectedColor", &QColorDialogWrap::selectedColor),
                     ...
                     StaticMethod("setCustomColor", &StaticQColorDialogWrapMethods::setCustomColor),
                     StaticMethod("setStandardColor", &StaticQColorDialogWrapMethods::setStandardColor),
                     StaticMethod("standardColor", &StaticQColorDialogWrapMethods::standardColor),
...
QColorDialog *QColorDialogWrap::getInternalInstance()
{
...
QColorDialogWrap::QColorDialogWrap(const Napi::CallbackInfo &info) : Napi::ObjectWrap<QColorDialogWrap>(info)
{
    ...
    else if (argCount == 1)
    {
        ...
        else
        {
            ...
            this->instance = new NColorDialog(parentWidgetWrap->getInternalInstance());
        }
    }
    else
    {
        Napi::TypeError::New(env, "Vixen: QColorDialogWrap: Wrong number of arguments to constructor")
...
Napi::Value QColorDialogWrap::selectedColor(const Napi::CallbackInfo &info)
{
    ...
    auto instance = QColorWrap::constructor.New({Napi::External<QColor>::New(env, new QColor(color))});
    return instance;
}
...
Napi::Value QColorDialogWrap::setOption(const Napi::CallbackInfo &info)
{
    ...
    this->instance->setOption(static_cast<QColorDialog::ColorDialogOption>(option), on);
    return env.Null();
}
...
Napi::Value QColorDialogWrap::testOption(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
    int option = info[0].As<Napi::Number>().Int32Value();
    bool on = this->instance->testOption(static_cast<QColorDialog::ColorDialogOption>(option));
...
{
    Napi::Env env = info.Env();
    Napi::TypeError::New(env, "Vixen: QColorDialog: done() is protected and can't be called.")
        .ThrowAsJavaScriptException();
    return env.Null();
}

Napi::Value StaticQColorDialogWrapMethods::customColor(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
...
Napi::Value StaticQColorDialogWrapMethods::customCount(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
...
Napi::Value StaticQColorDialogWrapMethods::setCustomColor(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
    int index = info[0].As<Napi::Number>().Int32Value();
...
Napi::Value StaticQColorDialogWrapMethods::setStandardColor(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
    int index = info[0].As<Napi::Number>().Int32Value();
    Napi::Object colorObject = info[1].As<Napi::Object>();
...
Napi::Value StaticQColorDialogWrapMethods::standardColor(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
    int index = info[0].As<Napi::Number>().Int32Value();
