...
QErrorMessageWrap::QErrorMessageWrap(const Napi::CallbackInfo &info) : Napi::ObjectWrap<QErrorMessageWrap>(info)
{
    ...
    else if (argCount == 1)
    {
        ...
        else
        {
            ...
            this->instance = new NErrorMessage(parentWidgetWrap->getInternalInstance());
        }
    }
    else
    {
        Napi::TypeError::New(env, "Vixen: QErrorMessageWrap: Wrong number of arguments to constructor")
...
{
    Napi::Env env = info.Env();
    Napi::TypeError::New(env, "Vixen: QErrorMessage: done() is protected and can't be called.")
        .ThrowAsJavaScriptException();
    return env.Null();
}
