...
QFontDialog *QFontDialogWrap::getInternalInstance()
{
...
QFontDialogWrap::QFontDialogWrap(const Napi::CallbackInfo &info) : Napi::ObjectWrap<QFontDialogWrap>(info)
{
    ...
    {
        Napi::TypeError::New(env, "Vixen: QFontDialogWrap: Wrong number of arguments to constructor")
...
Napi::Value QFontDialogWrap::setOption(const Napi::CallbackInfo &info)
{
    ...
    this->instance->setOption(static_cast<QFontDialog::FontDialogOption>(option), on);
    return env.Null();
}
...
Napi::Value QFontDialogWrap::testOption(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
    int option = info[0].As<Napi::Number>().Int32Value();
    bool on = this->instance->testOption(static_cast<QFontDialog::FontDialogOption>(option));
...
{
    Napi::Env env = info.Env();
    Napi::TypeError::New(env, "Vixen: QFontDialog: done() is protected and can't be called.")
        .ThrowAsJavaScriptException();
    return env.Null();
}
