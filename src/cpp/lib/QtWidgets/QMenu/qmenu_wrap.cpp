...
QMenu *QMenuWrap::getInternalInstance()
{
...
QMenuWrap::QMenuWrap(const Napi::CallbackInfo &info) : Napi::ObjectWrap<QMenuWrap>(info)
{
    ...
    {
        Napi::TypeError::New(env, "Vixen: QMenuWrap: Wrong number of arguments to constructor")
...
{
    Napi::Env env = info.Env();
    auto value = Napi::External<QAction>::New(env, this->instance->addSeparator());
...
Napi::Value QMenuWrap::insertSection_3(const Napi::CallbackInfo &info)
{
    ...
    QActionWrap *beforeWrap = Napi::ObjectWrap<QActionWrap>::Unwrap(info[0].As<Napi::Object>());
    QAction *before = beforeWrap->getInternalInstance();
    QIconWrap *iconWrap = Napi::ObjectWrap<QIconWrap>::Unwrap(info[1].As<Napi::Object>());
    QIcon *icon = iconWrap->getInternalInstance();
...
{
    Napi::Env env = info.Env();
    QPointWrap *posWrap = Napi::ObjectWrap<QPointWrap>::Unwrap(info[0].As<Napi::Object>());
