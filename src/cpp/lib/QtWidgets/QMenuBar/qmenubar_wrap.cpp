...
#include <vixen/Extras/Utils/nutils.h>
#include <vixen/QtWidgets/QWidget/qwidget_wrap.h>
...
Napi::Object QMenuBarWrap::init(Napi::Env env, Napi::Object exports)
{
    ...
    Napi::Function func = DefineClass(
        env, CLASSNAME,
        {InstanceMethod("addMenu", &QMenuBarWrap::addMenu), InstanceMethod("addSeparator", &QMenuBarWrap::addSeparator),
         ...
         InstanceMethod("addMenu_1", &QMenuBarWrap::addMenu_1), InstanceMethod("addMenu_2", &QMenuBarWrap::addMenu_2),
         InstanceMethod("clear", &QMenuBarWrap::clear), InstanceMethod("cornerWidget", &QMenuBarWrap::cornerWidget),
...
QMenuBar *QMenuBarWrap::getInternalInstance()
{
...
QMenuBarWrap::QMenuBarWrap(const Napi::CallbackInfo &info) : Napi::ObjectWrap<QMenuBarWrap>(info)
{
    ...
    {
        Napi::TypeError::New(env, "Vixen: QMenuBarWrap: Wrong number of arguments to constructor")
...
{
    Napi::Env env = info.Env();
    QPointWrap *ptWrap = Napi::ObjectWrap<QPointWrap>::Unwrap(info[0].As<Napi::Object>());
...
{
    Napi::Env env = info.Env();
    QActionWrap *actWrap = Napi::ObjectWrap<QActionWrap>::Unwrap(info[0].As<Napi::Object>());
...
{
    Napi::Env env = info.Env();
    Qt::Corner corner = static_cast<Qt::Corner>(info[0].As<Napi::Number>().Int32Value());
...
{
    Napi::Env env = info.Env();
    QActionWrap *beforeWrap = Napi::ObjectWrap<QActionWrap>::Unwrap(info[0].As<Napi::Object>());
    QAction *before = beforeWrap->getInternalInstance();
    QMenuWrap *menuWrap = Napi::ObjectWrap<QMenuWrap>::Unwrap(info[1].As<Napi::Object>());
