...
QMessageBox *QMessageBoxWrap::getInternalInstance()
{
...
QMessageBoxWrap::QMessageBoxWrap(const Napi::CallbackInfo &info) : Napi::ObjectWrap<QMessageBoxWrap>(info)
{
    ...
    {
        Napi::TypeError::New(env, "Vixen: QMessageBoxWrap: Wrong number of arguments to constructor")
...
Napi::Value QMessageBoxWrap::setDefaultButton(const Napi::CallbackInfo &info)
{
    ...
    QPushButtonWrap *buttonWrap = Napi::ObjectWrap<QPushButtonWrap>::Unwrap(buttonObject);
...
Napi::Value QMessageBoxWrap::addButton(const Napi::CallbackInfo &info)
{
    ...
    QAbstractButton *btn = qobject_cast<QAbstractButton *>(qobjWrap->getInternalInstance());
...
Napi::Value StaticQMessageBoxWrapMethods::about(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
    Napi::Object parentObject = info[0].As<Napi::Object>();
    NodeWidgetWrap *parentWidgetWrap = Napi::ObjectWrap<NodeWidgetWrap>::Unwrap(parentObject);
    ...
    QMessageBox::about(parentWidgetWrap->getInternalInstance(), QString::fromUtf8(title.c_str()),
                       QString::fromUtf8(text.c_str()));

    return env.Null();
}

Napi::Value StaticQMessageBoxWrapMethods::aboutQt(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
    Napi::Object parentObject = info[0].As<Napi::Object>();
    NodeWidgetWrap *parentWidgetWrap = Napi::ObjectWrap<NodeWidgetWrap>::Unwrap(parentObject);
    Napi::String napiTitle = info[1].As<Napi::String>();
    std::string title = napiTitle.Utf8Value();
    QMessageBox::aboutQt(parentWidgetWrap->getInternalInstance(), QString::fromUtf8(title.c_str()));
