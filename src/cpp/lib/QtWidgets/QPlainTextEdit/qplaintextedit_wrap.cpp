...
QPlainTextEditWrap::QPlainTextEditWrap(const Napi::CallbackInfo &info) : Napi::ObjectWrap<QPlainTextEditWrap>(info)
{
    ...
    else if (argCount == 1)
    {
        ...
        else
        {
            ...
            this->instance = new NPlainTextEdit(parentWidgetWrap->getInternalInstance());
        }
    }
    else
    {
        Napi::TypeError::New(env, "Vixen: QPlainTextEditWrap: Wrong number of arguments to constructor")
