...
QStackedLayoutWrap::QStackedLayoutWrap(const Napi::CallbackInfo &info) : Napi::ObjectWrap<QStackedLayoutWrap>(info)
{
    ...
    else if (argCount == 1)
    {
        ...
        else
        {
            ...
            this->instance = new NStackedLayout(parentWidgetWrap->getInternalInstance());
        }
    }
    else
    {
        Napi::TypeError::New(env, "Vixen: QStackedLayoutWrap: Wrong number of arguments to constructor")
...
Napi::Value QStackedLayoutWrap::addWidget(const Napi::CallbackInfo &info)
{
    ...
    NodeWidgetWrap *widget = Napi::ObjectWrap<NodeWidgetWrap>::Unwrap(qwidgetObject);
...
Napi::Value QStackedLayoutWrap::setCurrentIndex(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
...
Napi::Value QStackedLayoutWrap::indexOf(const Napi::CallbackInfo &info)
{
    ...
    return Napi::Number::New(env, static_cast<int>(this->instance->indexOf(widget->getInternalInstance())));
}

Napi::Value QStackedLayoutWrap::setStackingMode(const Napi::CallbackInfo &info)
{
    Napi::Env env = info.Env();
    int stackingModeInt = info[0].As<Napi::Number>().Int32Value();
    QStackedLayout::StackingMode stackingMode = static_cast<QStackedLayout::StackingMode>(stackingModeInt);
