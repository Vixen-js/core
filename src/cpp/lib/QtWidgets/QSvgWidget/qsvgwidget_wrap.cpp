...
Napi::Object QSvgWidgetWrap::init(Napi::Env env, Napi::Object exports)
{
    ...
    Napi::Function func = DefineClass(
        env, CLASSNAME,
        {InstanceMethod("load", &QSvgWidgetWrap::load), QWIDGET_WRAPPED_METHODS_EXPORT_DEFINE(QSvgWidgetWrap)});
...
QSvgWidget *QSvgWidgetWrap::getInternalInstance()
{
...
QSvgWidgetWrap::QSvgWidgetWrap(const Napi::CallbackInfo &info) : Napi::ObjectWrap<QSvgWidgetWrap>(info)
{
    ...
    {
        Napi::TypeError::New(env, "Vixen: QSvgWidgetWrap: Wrong number of arguments to constructor")
...
Napi::Value QSvgWidgetWrap::load(const Napi::CallbackInfo &info)
{
    ...
    if (s.startsWith(svgPrefix))
    {
        ...
        QByteArray svg = QByteArray::fromBase64(s.toUtf8(), QByteArray::Base64Encoding);
