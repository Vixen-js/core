...
QSystemTrayIconWrap::QSystemTrayIconWrap(const Napi::CallbackInfo &info) : Napi::ObjectWrap<QSystemTrayIconWrap>(info)
{
    ...
    else if (argCount == 1)
    {
        ...
        else
        {
            ...
            this->instance = new NSystemTrayIcon(parentWidgetWrap->getInternalInstance());
        }
    }
    else
    {
        Napi::TypeError::New(env, "Vixen: QSystemTrayIconWrap: Wrong number of "
...
Napi::Value QSystemTrayIconWrap::showMessage(const Napi::CallbackInfo &info)
{
    ...
    this->instance->showMessage(msgTitle, msgMessage, *iconWrap->getInternalInstance(), millis.Int32Value());
    return env.Null();
}