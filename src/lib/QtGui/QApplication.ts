...
The QApplication class manages the GUI application's control flow and main settings. In Vixen you will never create an instance of it manually. Vixen's internal runtime `Qode` does it for you on app start. You can access the initialised QApplication though if needed.
...
export class QApplication extends QObject<QApplicationSignals> {
  ...
  static screens(): QScreen[] {
    ...
    return screenNativeList.map((screenNative: any) =>
      wrapperCache.get<QScreen>(QScreen, screenNative)
    );
  }
