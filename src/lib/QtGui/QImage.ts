...
```javascript
import { QImage } from '@vixen-js/core';
...
export class QImage extends Component {
  ...
  pixelColor(pointOrX: QPoint | number, y?: number): QColor {
    const native =
      pointOrX instanceof QPoint
        ? this.native?.pixelColor(pointOrX.native)
        : this.native?.pixelColor(pointOrX, y);
  ...
  pixelIndex(pointOrX: QPoint | number, y?: number): number {
    const pixelIndex =
      pointOrX instanceof QPoint
        ? this.native?.pixelIndex(pointOrX.native)
        : this.native?.pixelIndex(pointOrX, y);
    return pixelIndex;
  }
  ...
  scaled(
    ...
    if (sizeOrWidth instanceof QSize) {
      native = this.native?.scaled(
        sizeOrWidth.native,
        modeOrHeight,
        transformOrAspectRatioMode
      );
    } else {
      native = this.native?.scaled(
        sizeOrWidth,
  ...
  setPixel(
    positionOrX: QPoint | number,
    indexOrRgbOrY: number,
    indexOrRgb?: number
  ): void {
  ...
  setPixelColor(
    ...
    this.native?.setPixelColor(
      positionOrX,
      colorOrY,
      color?.native as NativeElement
    );
  }
