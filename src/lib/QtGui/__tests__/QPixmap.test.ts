...
const testImagePath = path.resolve(__dirname, "assets", "Logo-Color.jpg");
describe("QPixmap", () => {
  it("initialize empty", () => {
  ...
  it("save to a file", async () => {
    const outputFilePath = path.resolve(__dirname, "assets", "Logo-Color.png");
    await new Promise((resolve) => fs.unlink(outputFilePath, resolve));
    const pixmap = new QPixmap(testImagePath);
    pixmap.save(outputFilePath);
    const exists = await new Promise((resolve) =>
      fs.exists(outputFilePath, resolve)
    );
    expect(exists).toBe(true);
  });
  it("save to a file of different format", async () => {
    const outputFilePath = path.resolve(__dirname, "assets", "Logo-Color.jpg");
    await new Promise((resolve) => fs.unlink(outputFilePath, resolve));
    const pixmap = new QPixmap(testImagePath);
