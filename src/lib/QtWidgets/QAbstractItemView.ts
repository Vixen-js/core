...
export abstract class QAbstractItemView<
  Signals extends QAbstractItemViewSignals
> extends QAbstractScrollArea<Signals> {
  // *** Public Functions ***
  ...
  selectionModel(): QItemSelectionModel {
    return wrapperCache.getWrapper(
      this.native?.selectionModel()
    ) as QItemSelectionModel;
  }
  ...
  setItemDelegate(delegate: QAbstractItemDelegate): void {
    if (delegate != null) {
      ...
      if (parent != null && parent != this) {
        throw new Error(
          "Vixen: QAbstractItemDelegate instances can only be given to one view instance."
        );
      }
