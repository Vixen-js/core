...
QAbstractScrollArea will list all methods and properties that are common to all scrollable widgets in the Vixen world.

 */
export abstract class QAbstractScrollArea<
  Signals extends QAbstractScrollAreaSignals
> extends QFrame<Signals> {
  horizontalScrollBar(): QScrollBar {
    return wrapperCache.getWrapper(
      this.native?.horizontalScrollBar()
    ) as QScrollBar;
  }
