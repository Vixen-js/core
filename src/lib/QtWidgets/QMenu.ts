...
export class QMenu extends QWidget<QMenuSignals> {
  ...
  addMenu(titleOrIcon: string | QIcon, title?: string): QMenu {
    if (typeof titleOrIcon === "string") {
      ...
      return wrapperCache.getWrapper(
        this.native?.addMenu_2(titleOrIcon.native, title)
      ) as QMenu;
    }
  }
  ...
  addSection(textOrIcon: string | QIcon, text?: string): QAction {
    if (typeof textOrIcon === "string") {
      return wrapperCache.getWrapper(
        this.native?.addSection_1(textOrIcon)
      ) as QAction;
    } else {
      return wrapperCache.getWrapper(
        this.native?.addSection_2(textOrIcon.native, text)
      ) as QAction;
    }
  }
  ...
  insertSection(
    before: QAction,
    textOrIcon: string | QIcon,
    text?: string
  ): QAction {
    if (typeof textOrIcon === "string") {
      return wrapperCache.getWrapper(
        this.native?.insertSection_2(before.native, text)
      ) as QAction;
    } else {
      return wrapperCache.getWrapper(
  ...
  showTearOffMenu(pos: QPoint | null = null): void {
    if (pos == null) {
