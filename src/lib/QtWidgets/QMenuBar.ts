...
```javascript
import { QMenuBar, QMainWindow } from "@vixen-js/core";
...
export class QMenuBar extends QWidget<QMenuBarSignals> {
  ...
  addMenu(menuOrStringOrIcon: QMenu | QIcon | string, title?: string): QMenu {
    if (typeof menuOrStringOrIcon === "string") {
      return wrapperCache.getWrapper(
        this.native?.addMenu_1(menuOrStringOrIcon)
      ) as QMenu;
    }
    if (menuOrStringOrIcon instanceof QMenu) {
      return wrapperCache.getWrapper(
        this.native?.addMenu(menuOrStringOrIcon.native)
      ) as QMenu;
    }
    return wrapperCache.getWrapper(
      this.native?.addMenu_2(menuOrStringOrIcon.native, title)
    ) as QMenu;
  }
  ...
  cornerWidget(corner: Corner = Corner.TopRightCorner): QWidget {
    return wrapperCache.getWrapper(
      this.native?.cornerWidget(corner)
    ) as QWidget;
  }
  insertMenu(before: QAction, menu: QMenu): QAction {
    return wrapperCache.getWrapper(
      this.native?.insertMenu(before.native, menu.native)
    ) as QAction;
  }
  insertSeparator(before: QAction): QAction {
    return wrapperCache.getWrapper(
      this.native?.insertSeparator(before.native)
    ) as QAction;
  }
  ...
  setCornerWidget(
    widget: QWidget,
    corner: Corner = Corner.TopRightCorner
  ): void {
