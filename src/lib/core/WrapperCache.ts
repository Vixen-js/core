...
export class WrapperCache {
  ...
  private _objectDestroyedCallback(objectId: number): void {
    if (this._strongCache.has(objectId)) {
      ...
      if (this.logDestoryQObject) {
        console.log(`Vixen: Destroyed C++ object with ID: ${objectId}.`);
      }
      return;
    }
  ...
  get<T extends QObject>(
    wrapperConstructor: { new (native: any): T },
  ...
  getWrapper(native: any): QObject | null {
    ...
    if (this._strongCache.has(id)) {
      return this._strongCache.get(id) ?? null;
    }
    ...
    if (this._wrapperRegistry.has(native.wrapperType)) {
      ...
      console.log(
        `Vixen: Unable to find JS wrapper for type '${native.wrapperType}'.`
      );
    }
  ...
  registerWrapper(
    ...
    wrapperConstructor: { new (native: any): QObject }
  ): void {
  ...
  store(wrapper: QObject): void {
    if (wrapper.native != null) {
      ...
      addon.WrapperCache_store(
        wrapper.native,
        wrapper.native?.__external_qobject__()
      );
      if (this.logCreateQObject) {
        console.log(`Vixen: Created C++ object with ID: ${objectId}.`);
      }
    }
  }
}
